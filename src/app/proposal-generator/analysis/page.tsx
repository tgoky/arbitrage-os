...
export default function AnalysisDetailPage() {
  ...
  const handleQuickGenerate = () => {
    ...
    // Attach full raw deal architecture for maximum AI context
    const input = {
      ...prefill,
      rawAnalysisContext: JSON.stringify(analysis?.analysis?.dealArchitecture),
    };
    try {
      sessionStorage.setItem('proposal_generator_prefill', JSON.stringify(input));
    } catch {
      message.error('Failed to prepare proposal data.');
      return;
    }
    go({ to: '/proposal-generator/result' });
  };
