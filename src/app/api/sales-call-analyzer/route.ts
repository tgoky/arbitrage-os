...
async function getAuthenticatedUser(request: NextRequest) {
  try {
    ...
    try {
      ...
      if (!error && user) {
        console.log('✅ Sales Call Analyzer Auth Method 1 (route handler) succeeded for user:', user.id);
        return { user, error: null };
      }
      
      console.log('⚠️ Sales Call Analyzer route handler auth failed:', error?.message);
    } catch (helperError) {
      console.warn('⚠️ Sales Call Analyzer route handler client failed:', helperError);
    }
    ...
    if (authHeader?.startsWith('Bearer ')) {
      try {
        ...
        console.log('🔍 Sales Call Analyzer trying token auth with token:', token.substring(0, 20) + '...');
        ...
        if (!error && user) {
          console.log('✅ Sales Call Analyzer Auth Method 2 (token) succeeded for user:', user.id);
          return { user, error: null };
        }
        
        console.log('⚠️ Sales Call Analyzer token auth failed:', error?.message);
      } catch (tokenError) {
        console.warn('⚠️ Sales Call Analyzer token auth error:', tokenError);
      }
    }
            ...
            try {
              ...
              if (cookie.value.startsWith('base64-')) {
                try {
                  ...
                  console.warn(`Invalid Sales Call Analyzer cookie ${name}, skipping...`);
                  return undefined;
                }
              }
              return cookie.value;
            } catch (error) {
              console.warn(`Error reading Sales Call Analyzer cookie ${name}:`, error);
              return undefined;
            }
          },
        },
    ...
    if (!error && user) {
      console.log('✅ Sales Call Analyzer Auth Method 3 (SSR cookies) succeeded for user:', user.id);
    } else {
      console.log('⚠️ Sales Call Analyzer SSR cookie auth failed:', error?.message);
    }
    
    return { user, error };
    
  } catch (error) {
    console.error('💥 All Sales Call Analyzer authentication methods failed:', error);
