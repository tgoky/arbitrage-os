...
const NicheResearchDetailPage = () => {
  ...
  const downloadResearch = async (format: "html" | "json" = "html") => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(result.error || "Failed to export research");
  ...
  const report = researchDetail.report;
  const meta = researchDetail.metadata;
  const recommendedIndex = meta.recommendedNicheIndex || 0;
  ...
  return (
    <div className="max-w-4xl mx-auto px-8 py-14" style={{ fontFamily: "'Manrope', sans-serif" }}>
      ...
      {/* Title */}
      <h1 className="text-3xl font-semibold text-gray-100 mb-2 leading-tight">{researchDetail.title}</h1>
      <p className="text-base text-gray-500 mb-2">
        {new Date(researchDetail.createdAt).toLocaleDateString()}
        {meta.primaryObjective && <> &middot; {meta.primaryObjective}</>}
        {meta.marketType && <> &middot; {meta.marketType}</>}
      </p>

      {/* Meta pills */}
      <div className="flex flex-wrap gap-2 mb-4">
        {meta.budget && <Pill>{meta.budget}</Pill>}
        {meta.customerSize && <Pill>{meta.customerSize}</Pill>}
        {meta.riskAppetite && <Pill>{meta.riskAppetite} risk</Pill>}
        {(meta.skills || []).slice(0, 3).map((s, i) => <Pill key={i}>{s}</Pill>)}
        {niches.length > 1 && <Pill>{niches.length} niches compared</Pill>}
      </div>

      {/* Research metadata */}
      {(meta.industries?.length || meta.geographicFocus || meta.teamSize || meta.timeCommitment) && (
        <div className="space-y-1.5 text-sm text-gray-500 mb-4">
          {meta.industries && meta.industries.length > 0 && (
            <p><span className="text-gray-600">Industries:</span> {meta.industries.join(", ")}</p>
          )}
          {meta.geographicFocus && <p><span className="text-gray-600">Geographic Focus:</span> {meta.geographicFocus}</p>}
          {meta.teamSize && <p><span className="text-gray-600">Team Size:</span> {meta.teamSize}</p>}
          {meta.timeCommitment && <p><span className="text-gray-600">Time Commitment:</span> {meta.timeCommitment}</p>}
        </div>
      )}
        ...
        <>
          <div className="text-center mb-10">
            ...
            <p className="text-2xl font-light text-gray-100 mb-2">
              {niches[recommendedIndex]?.nicheOverview?.name || "N/A"}
            </p>
            <p className="text-base text-gray-400 max-w-2xl mx-auto leading-relaxed">
              {report.recommendationReason || "No recommendation reason provided."}
            </p>
          </div>
          ...
          {report.comparisonMatrix?.scores?.length > 0 && (
            <div className="mb-10">
              ...
              <div className="space-y-3">
                {report.comparisonMatrix.scores.map((score, i) => {
                  const niche = niches[score.nicheIndex ?? i];
                  const isRecommended = (score.nicheIndex ?? i) === recommendedIndex;
                  return (
      ...
      {niches.length > 1 ? (
        ...
        <NicheDetail data={niches[0]} />
      ) : (
        <p className="text-base text-gray-500 py-8">No niche data available.</p>
      )}
