...
import { useNicheResearcher } from "../hooks/useNicheResearcher"; // Import useNicheResearcher
...
export const SavedNicheHistory = () => {
  ...
  const { getNicheReport } = useNicheResearcher(); // Add getNicheReport
  const { currentWorkspace, isWorkspaceReady } = useWorkspaceContext();
  const [isModalVisible, setIsModalVisible] = useState(false);
  const [selectedNiche, setSelectedNiche] = useState<SavedNiche | null>(null);
  const [modalLoading, setModalLoading] = useState(false); // Track modal loading
  const [modalError, setModalError] = useState<string | null>(null); // Track modal errors
  ...
  const showNicheDetails = async (niche: SavedNiche) => {
    console.log("Selected Niche:", niche); // Debug log
    setModalLoading(true);
    setModalError(null);
    try {
      // Fetch full report to ensure complete data
                    ...
                    <Space>
                      <Tooltip title="View this niche report">
                        <Button
                          ...
                          icon={<EyeOutlined />}
                          onClick={() => showNicheDetails(niche)}
      ...
      >
        {modalLoading ? (
          ...
          <Alert
            ...
            showIcon
          />
        ) : selectedNiche && selectedNiche.content && selectedNiche.content.niches?.length > 0 ? (
          <div style={{ maxHeight: "70vh", overflowY: "auto" }}>
            {renderDetailedNicheReport(
              selectedNiche.content.niches[selectedNiche.content.recommendedNiche] || {},
              selectedNiche.id
            )}
          </div>
        ) : (
